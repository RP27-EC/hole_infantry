...
void KT_motor_class_init(KT_motor_t *motor)
{
    ...
    motor->KT_motor_info.state_info.offline_cnt_max = 200;
    motor->KT_motor_info.state_info.selfprotect_cnt_max = 200;
...
void kt_motor_multi_control(int16_t *iqControl, char kt_motor_num, motor_drive_e drive_type)
{
    ...
    {
        CAN_SendData(&hfdcan1, KT_MULTI_TX_ID, tx_buff);
    }
    else if (drive_type == M_CAN2)
    {
        CAN_SendData(&hfdcan2, KT_MULTI_TX_ID, tx_buff);
    }
    else if (drive_type == M_CAN3)
    {
        CAN_SendData(&hfdcan3, KT_MULTI_TX_ID, tx_buff);
    }
    else
        return;
}
...
void tx_kt_motor_W_command(KT_motor_t *motor, uint8_t command)
{
    ...
    {
        CAN_SendData(&hfdcan1, motor->KT_motor_info.id.tx_id, motor->tx_buff);
    }
    else if (motor->KT_motor_info.id.drive_type == M_CAN2)
    {
        CAN_SendData(&hfdcan2, motor->KT_motor_info.id.tx_id, motor->tx_buff);
    }
    else if (motor->KT_motor_info.id.drive_type == M_CAN3)
    {
        CAN_SendData(&hfdcan3, motor->KT_motor_info.id.tx_id, motor->tx_buff);
    }
    else
        return;
}
