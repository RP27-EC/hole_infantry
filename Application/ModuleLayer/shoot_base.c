...
/*外部可通过shoot访问内部所有数据与命令*//*763到787自己加上的*/
Shoot_t shoot;
...
void Shoot_Init(Shoot_t* shoot)
{
	...
	shoot->info.cfg_rx_info.base_cfg_info.speed_stop_mode =0.1;         
	
	shoot->info.cfg_rx_info.base_cfg_info.stop_angle_err_max =0.1f;
	                                                                                     
	//拨盘复位堵转配置                            
	shoot->info.cfg_rx_info.reset_speed_block_cfg_info.block_judge_type =0;
	                                      
	shoot->info.cfg_rx_info.reset_speed_block_cfg_info.speed_max =0.3;
	shoot->info.cfg_rx_info.reset_speed_block_cfg_info.current_min =5;
	shoot->info.cfg_rx_info.reset_speed_block_cfg_info.block_time_max =100;
	
	shoot->info.cfg_rx_info.reset_speed_block_cfg_info.integral_value =1;
	...
	switch (shoot->cmd.dial_tx_cmd.work_state)
	{
		...
		case RESETING:                                            //复位状态更新
		...
		#if DIAL_IS_ABSOLUTE_ANGLE == 0              
			{
			  ...
			  {
			  	//对拨盘回转调整角度限幅，防止过大
				  if(my_abs(shoot->info.cfg_rx_info.base_cfg_info.reset_adjust_angle) > my_abs(shoot->info.cfg_rx_info.base_cfg_info.oneshot_angle))
				  {
