...
//拨盘
#define  DIAL_MOTOR_TYPE                  DM_J4310                  //拨盘电机类型，从Dial_Motor_Type_e里面选

#define  DIAL_MEC_LIMIT                   1                        //拨盘有无机械限位，无为 -1，有为 1

#define  DIAL_IS_ABSOLUTE_ANGLE           0                        //拨盘是否有绝对角度，有为 1，没有为 0

#define  DIAL_PUSHER_NUM                  9                        //拨盘拨爪数量

#define  DIAL_ANGLE_MAX                    PI                  //拨盘机械角度数值最大值，如相对角度有8191，绝对角度有10.f
#define  DIAL_ANGLE_MIN                    -PI                     //拨盘机械角度数值最小值，如 相对角度的0，绝对角度有的是-10.f

#define  DIAL_ANGLE_DATA_TYPE             float              //拨盘角度数据类型
#define  DIAL_SPEED_DATA_TYPE             float                  //拨盘速度数据类型
#define  DIAL_CURRENT_DATA_TYPE           float                  //拨盘电流数据类型

#define  DIAL_ANGLE_SUM_DATA_TYPE         float                  //拨盘角度和数据类型


//摩擦轮
#define  FRIC_NUM                         3                        //摩擦轮数量，分六摩 6，三摩 3，二摩 2

#define  FRIC_SPEED_DATA_TYPE             float                  //摩擦轮速度数据类型
#define  FRIC_CURRENT_DATA_TYPE           float                  //摩擦轮电流数据类型
...
typedef struct{
	...
	uint8_t fire_mode_flag;                 //开火模式标志位，单发为 0，连发为 1              0就好了
	uint8_t elec_level_flag;                //电平标志位，高电平为 1，低电平为 0            1才会开火，关联跳变前为0，跳变后为1
